...
// Generate CSS variable for the theme color dynamically
...
<template>
  <div class="print-container bg-white text-slate-800 w-[210mm] min-h-[297mm] print:w-full print:max-w-none print:min-h-0 mx-auto p-10 md:p-14 print:px-14 print:py-12 block text-[13px] md:text-[14px] leading-relaxed" :style="cssVars">
    ...
    <!-- HEADER -->
    <header id="section-personal" class="mb-5 pb-3 border-b-2 flex justify-between items-end gap-4" style="border-color: var(--theme-color);">
      <div class="pb-1">
        <h1 class="text-3xl font-bold text-slate-900 leading-none mb-2">
          {{ data.personal.name }} <span v-if="data.personal.nameZh" class="font-sans ml-1 text-2xl font-semibold">({{ data.personal.nameZh }})</span>
        </h1>
        <p v-if="data.personal.showDocumentTitle !== false" class="font-medium" style="color: var(--theme-color)">{{ data.personal.documentTitle || 'Curriculum Vitae' }}</p>
      </div>
      
      <div class="flex flex-col text-[11px] text-slate-500 space-y-1 text-right pb-1">
        <p v-if="data.personal.phone" class="flex gap-1.5 items-center justify-end"><i class="fa-solid fa-phone text-[10px] w-3 text-center"></i> {{ data.personal.phone }}</p>
        <p v-if="data.personal.email" class="flex gap-1.5 items-center justify-end"><i class="fa-solid fa-envelope text-[10px] w-3 text-center"></i> <a :href="'mailto:' + data.personal.email" class="hover:underline">{{ data.personal.email }}</a></p>
        <p v-if="data.personal.github" class="flex gap-1.5 items-center justify-end"><i class="fa-brands fa-github text-[11px] w-3 text-center"></i> <a :href="data.personal.githubUrl" target="_blank" class="hover:underline">{{ data.personal.githubUrl.replace('https://', '') }}</a></p>
        <p v-if="data.personal.website" class="flex gap-1.5 items-center justify-end"><i class="fa-solid fa-link text-[10px] w-3 text-center"></i> <a :href="data.personal.websiteUrl" target="_blank" class="hover:underline">{{ data.personal.websiteUrl.replace('https://', '') }}</a></p>
      </div>
    </header>

    <template v-for="secId in (data.sectionOrder || ['education', 'employment', 'skills', 'articles', 'conferences', 'academic', 'awards'])" :key="secId">
      <!-- SECTION: EDUCATION -->
      <section :id="'section-' + secId" v-if="secId === 'education' && data.education.length > 0 && data.sectionSettings?.education?.visible !== false" class="mb-6">
        <h2 class="text-xs font-bold uppercase tracking-widest border-l-4 pl-3 mb-4 flex items-center gap-2" style="color: var(--theme-color); border-color: var(--theme-color);">
      ...
      <!-- SECTION: EMPLOYMENT -->
      <section :id="'section-' + secId" v-else-if="secId === 'employment' && data.employment.length > 0 && data.sectionSettings?.employment?.visible !== false" class="mb-6">
        <h2 class="text-xs font-bold uppercase tracking-widest border-l-4 pl-3 mb-4 flex items-center gap-2" style="color: var(--theme-color); border-color: var(--theme-color);">
      ...
      <!-- SECTION: SKILLS -->
      <section :id="'section-' + secId" v-else-if="secId === 'skills' && data.skills.length > 0 && data.sectionSettings?.skills?.visible !== false" class="mb-6 break-inside-avoid">
        <h2 class="text-xs font-bold uppercase tracking-widest border-l-4 pl-3 mb-4 flex items-center gap-2" style="color: var(--theme-color); border-color: var(--theme-color);">
      ...
      <!-- SECTION: ARTICLES -->
      <section :id="'section-' + secId" v-else-if="secId === 'articles' && data.articles.length > 0 && data.sectionSettings?.articles?.visible !== false" class="mb-6 break-inside-avoid">
        <h2 class="text-xs font-bold uppercase tracking-widest border-l-4 pl-3 mb-4 flex items-center gap-2" style="color: var(--theme-color); border-color: var(--theme-color);">
              ...
              <div>
                <p class="text-sm text-slate-900 leading-snug">
                  ...
                  <template v-for="(au, idx) in ar.authors" :key="idx">
                    <span :class="{'font-semibold underline decoration-2': au.isMe, 'decoration-[#01a3a4]': au.isMe}" :style="au.isMe ? `text-decoration-color: var(--theme-color)` : ''">{{ au.name }}</span>
                  ...
                  <span class="font-semibold ml-1">{{ ar.journal }}</span> <template v-if="ar.year">{{ ar.year }}.</template>
                  <template v-if="ar.volumeAndIssue"> {{ ar.volumeAndIssue }}</template><template v-if="ar.pages">, {{ ar.pages }}.</template>
                  
                  <!-- Inline links and badges -->
                  <span class="inline-flex items-center gap-1.5 ml-1.5 align-middle">
                    <a v-if="ar.doi" :href="'https://doi.org/' + ar.doi" target="_blank" class="text-[10px] text-blue-600 hover:underline">doi:{{ ar.doi }}</a>
                    <span v-if="ar.impactFactor !== undefined" class="inline-flex items-center relative">
                      <img :src="`https://img.shields.io/badge/${ar.impactFactor === 0 ? 'Chapter' : 'IF-' + ar.impactFactor.toFixed(1)}-%235A8693?style=flat-square&logoColor=white&color=%23${ar.impactFactor === 0 ? '0A203F' : 'F46A6A'}`" alt="Impact Factor" class="h-[14px] pointer-events-none" />
                    </span>
                  </span>
                </p>
              </div>
      ...
      <section :id="'section-' + secId" v-else-if="secId === 'conferences' && data.conferences.length > 0 && data.sectionSettings?.conferences?.visible !== false" class="mb-6 break-inside-avoid">
        <h2 class="text-xs font-bold uppercase tracking-widest border-l-4 pl-3 mb-4 flex items-center gap-2" style="color: var(--theme-color); border-color: var(--theme-color);">
      ...
      <section :id="'section-' + secId" v-else-if="secId === 'academic' && (data.societyServices.length > 0 || data.reviews.length > 0 || data.contributions.length > 0 || data.fundings.length > 0) && data.sectionSettings?.academic?.visible !== false" class="mb-6">
        <h2 class="text-xs font-bold uppercase tracking-widest border-l-4 pl-3 mb-4 flex items-center gap-2" style="color: var(--theme-color); border-color: var(--theme-color);">
        ...
        <div v-if="data.societyServices.length > 0" class="mb-4 pl-1">
          ...
          <ul class="space-y-1">
            <li v-for="(ss, idx) in data.societyServices" :key="idx" class="text-xs text-slate-600 pl-[1em] -indent-[1em]">
        ...
        <div v-if="data.reviews.length > 0 || data.contributions.length > 0" class="mb-4 pl-1">
          ...
          <ul class="space-y-1">
            <li v-for="(rv, idx) in data.reviews" :key="'rv'+idx" class="text-xs text-slate-600 pl-[1em] -indent-[1em]">
              <span class="inline-block w-[1em] text-center !indent-0">•</span> {{ rv }}
            </li>
            <li v-for="(ct, idx) in data.contributions" :key="'ct'+idx" class="text-xs text-slate-600 pl-[1em] -indent-[1em]">
      ...
      <!-- SECTION: AWARDS -->
      <section :id="'section-' + secId" v-else-if="secId === 'awards' && data.awards.length > 0 && data.sectionSettings?.awards?.visible !== false" class="mb-6">
        <h2 class="text-xs font-bold uppercase tracking-widest border-l-4 pl-3 mb-4 flex items-center gap-2" style="color: var(--theme-color); border-color: var(--theme-color);">
              ...
              <p class="text-xs text-slate-600 leading-snug">
                ...
                <span v-if="aw.links && aw.links.length > 0" class="inline-flex gap-2 ml-1">
                  <template v-for="(link, i) in aw.links" :key="i">
                     <a :href="link.url" target="_blank" class="text-blue-600 hover:underline border-b border-transparent hover:border-blue-600 text-[10px]">[{{ link.label }}]</a>
                  </template>
                </span>
              </p>
           </div>
      ...
      <section :id="'section-' + secId" v-else-if="secId.startsWith('custom-') && data.customSections?.find(c => 'custom-'+c.id === secId)?.visible && (data.customSections?.find(c => 'custom-'+c.id === secId)?.items?.length || 0) > 0" class="mb-6">
        <h2 class="text-xs font-bold uppercase tracking-widest border-l-4 pl-3 mb-4 flex items-center gap-2" style="color: var(--theme-color); border-color: var(--theme-color);">
        ...
        <div class="space-y-4 pl-1">
           <div v-for="item in data.customSections?.find(c => 'custom-'+c.id === secId)?.items" :key="item.id">
              ...
              <ul v-if="item.bullets && item.bullets.length > 0" class="space-y-1 mt-1">
                <li v-for="(bullet, bIdx) in item.bullets" :key="bIdx" class="text-xs text-slate-600 pl-[1em] -indent-[1em]">
    ...
    <div v-if="data.personal.showFooterTitle !== false || data.personal.showLastModified !== false" class="mt-8 pt-4 border-t border-slate-100 flex justify-between text-[8px] text-slate-300 uppercase tracking-widest print:hidden mt-auto">
